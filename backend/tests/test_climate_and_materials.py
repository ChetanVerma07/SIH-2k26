...
def test_get_current_weather_falls_back_to_preset_without_api_key(client):
    resp = client.get("/api/v1/climate/current/Ladakh")
    assert resp.status_code == 200
    body = resp.json()
    assert body["location"] == "Ladakh"
    assert body["data_source"] == "weather_api_not_configured"
