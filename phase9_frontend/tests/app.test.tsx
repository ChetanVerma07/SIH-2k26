...
describe('Navigation', () => {
  ...
  it('navigates to New Analysis via sidebar', () => {
    renderApp('/');
    fireEvent.click(screen.getByRole('link', { name: /new analysis/i }));
    expect(screen.getByRole('heading', { name: /new analysis/i })).toBeInTheDocument();
  });

  it('navigates to Materials page', () => {
    renderApp('/');
    fireEvent.click(screen.getByRole('link', { name: /materials/i }));
...
describe('Design configuration', () => {
  it('toggles between auto optimize and manual design modes', () => {
    ...
    expect(screen.getByLabelText(/^length \(m\)$/i)).not.toBeDisabled();
  });
});
